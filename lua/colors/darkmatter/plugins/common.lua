-- highlight support for common plugins, including:
-- * nvim-tree
-- * NeoTree
-- * cmp
-- * GitSigns
-- * Telescope
-- * Mini.pick
-- * Navic
-- * Aerial
-- * Navbuddy
-- * Cokeline

local function _set()
  local c = require("colors.darkmatter")
  local conf = c.get_conf()

  c.hl_with_defaults("NvimTreeNormal", c.localtheme.fg_dim, c.localtheme.neotreebg)
  c.hl_with_defaults("NvimTreeEndOfBuffer", c.localtheme.bg_dim, c.localtheme.neotreebg)
  c.hl_with_defaults("NvimTreeVertSplit", c.localtheme.bg0, c.localtheme.bg0)
  c.link("NvimTreeSymlinkFolderName", "Blue")
  c.link("NvimTreeSymlinkFolderIcon", "BlueBold")
  c.link("NvimTreeFolderName", "Green")
  c.link("NvimTreeRootFolder", "Darkyellow")
  c.link("NvimTreeFolderIcon", "GreenBold")
  c.link("NeoTreeDirectoryIcon", "GreenBold")
  c.link("NeoTreeDirectoryName", "GreenBold")

  c.link("NvimTreeEmptyFolderName", "Fg")
  c.link("NvimTreeOpenedFolderName", "GreenBold")
  c.link("NvimTreeExecFile", "Fg")
  c.link("NvimTreeOpenedFile", "Orange")
  c.link("NvimTreeSpecialFile", "Fg")
  c.link("NvimTreeImageFile", "Fg")
  c.link("NvimTreeMarkdownFile", "Fg")
  c.link("NvimTreeIndentMarker", "SymbolsOutlineConnector")
  c.link("NvimTreeFolderArrowOpen", "SymbolsOutlineConnector")
  c.link("NvimTreeFolderArrowClosed", "SymbolsOutlineConnector")
  c.link("NvimTreeGitDirty", "Yellow")
  c.link("NvimTreeGitStaged", "Blue")
  c.link("NvimTreeGitMerge", "Orange")
  c.link("NvimTreeGitRenamed", "Purple")
  c.link("NvimTreeGitNew", "Green")
  c.link("NvimTreeGitDeleted", "Red")
  c.link("NvimTreeLspDiagnosticsError", "RedSign")
  c.link("NvimTreeLspDiagnosticsWarning", "YellowSign")
  c.link("NvimTreeLspDiagnosticsInformation", "BlueSign")
  c.link("NvimTreeLspDiagnosticsHint", "GreenSign")
  c.link("NvimTreeCursorLine", "TreeCursorLine")

  c.link("MiniNotifyNormal", "NeoTreeNormalNC")
  c.link("MiniNotifyBorder", "CmpBorder")
  c.link("NeoTreeFilenameOpened", "OrangeBold")
   -- CMP (with custom menu setup)
  c.link("CmpItemMenu", "Fg")
  c.link("CmpItemMenuDetail", "@include")
  c.link("CmpItemMenuBuffer", "FgDim")
  c.link("CmpItemMenuSnippet", "Number")
  c.link("CmpItemMenuLSP", "StorageClass")
  c.link("CmpItemAbbr", "Fg")
  c.set_hl(0, "CmpItemAbbrDeprecated", { bg = "NONE", strikethrough = true, fg = "#808080" })
  c.link("CmpItemAbbrMatch", "RedBold")
  c.link("CmpItemAbbrMatchFuzzy", "DarkPurpleBold")
  c.hl_with_defaults("CmpPmenu", c.localtheme.fg, c.localtheme.bg_dim)
  c.hl_with_defaults("CmpPmenuBorder", c.localtheme.grey_dim, c.localtheme.bg_dim)
  c.hl_with_defaults("CmpGhostText", c.localtheme.grey, c.NONE)
  c.link("CmpItemKindDefault", "FgDim")
  c.link("CmpItemKind", "CmpItemKindDefault")
  c.link("CmpItemMenuPath", "CmpItemMenu")
  c.link("CmpItemKindStruct", "Structure")
  c.link("CmpItemKindConstructor", "@constructor")
  c.link("CmpItemKindMethod", "Method")
  c.link("CmpItemKindModule", "@include")
  c.link("CmpItemKindClass", "Class")
  c.link("CmpItemKindVariable", "Fg")
  c.link("CmpItemKindProperty", "Member")
  c.link("CmpItemKindField", "Member")
  c.link("CmpItemKindFunction", "Function")
  c.link("CmpItemKindKeyword", "Keyword")
  c.link("CmpItemKindText", "String")
  c.link("CmpItemKindUnit", "@include")
  c.link("CmpItemKindConstant", "Constant")
  c.link("CmpItemKindEnum", "CmpItemKindConstant")
  c.link("CmpItemKindSnippet", "CmpItemMenuSnippet")
  c.link("CmpItemKindOperator", "Operator")
  c.link("CmpItemKindInterface", "Interface")
  c.link("CmpItemKindValue", "StorageClass")
  c.link("CmpItemKindTypeParameter", "Type")

  c.hl_with_defaults("CmpFloat", c.localtheme.fg_dim, c.localtheme.neotreebg)
  c.hl_with_defaults("CmpBorder", c.localtheme.accent, c.localtheme.neotreebg)

  c.hl_with_defaults("TelescopeBorder", c.localtheme.accent, c.localtheme.neotreebg)
  c.hl_with_defaults("TelescopePromptBorder", c.localtheme.accent, c.localtheme.neotreebg)
  c.hl_with_defaults("TelescopeNormal", c.localtheme.fg_dim, c.localtheme.neotreebg)
  c.hl("TelescopeTitle", c.localtheme.accent_fg, c.localtheme.accent, conf.attrib.bold)
  c.hl("TelescopePromptNormal", c.localtheme.fg_dim, c.localtheme.neotreebg, conf.attrib.bold)
  c.hl("TelescopeMatching", c.localtheme.deepred, c.NONE, conf.attrib.bold)

  c.link("MiniPickBorder", "TelescopeBorder")
  c.link("MiniPickBorderBusy", "TelescopeBorder")
  c.link("MiniPickBorderText", "TelescopeBorder")
  c.link("MiniPickNormal", "TelescopeNormal")
  c.link("MiniPickHeader", "TelescopeTitle")
  c.link("MiniPickMatchCurrent", "Visual")
  c.link("TelescopeResultsLineNr", "Yellow")
  c.link("TelescopePromptPrefix", "Blue")
  c.link("TelescopeSelection", "TreeCursorLine")
  c.link("FzfLuaNormal", "TelescopeNormal")
  c.link("FzfLuaBorder", "TelescopeBorder")
  c.link("FzfLuaSearch", "TelescopeMatching")

  c.link("GitSignsAdd", "GreenSign")
  c.link("GitSignsAddNr", "GreenSign")
  c.link("GitSignsChange", "BlueSign")
  c.link("GitSignsChangeNr", "BlueSign")
  c.link("GitSignsDelete", "RedSign")
  c.link("GitSignsDeleteNr", "RedSign")
  c.link("GitSignsAddLn", "GreenSign")
  c.link("GitSignsChangeLn", "BlueSign")
  c.link("GitSignsDeleteLn", "RedSign")
  c.link("GitSignsCurrentLineBlame", "Grey")
  c.link("GitSignsAddInline", "Visual")
  c.link("GitSignsChangeInline", "Visual")
  c.link("GitSignsDeleteInline", "Visual")

  c.link("TroubleNormal", "NeoTreeNormalNC")

  vim.api.nvim_set_hl(0, "NavicIconsFile",          {bg = c.localtheme.accent[1], fg = c.localtheme.fg[1]})
  vim.api.nvim_set_hl(0, "NavicIconsModule",        {bg = c.localtheme.accent[1], fg = c.localtheme.olive[1]})
  vim.api.nvim_set_hl(0, "NavicIconsNamespace",     {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsPackage",       {bg = c.localtheme.accent[1], fg = c.localtheme.olive[1]})
  vim.api.nvim_set_hl(0, "NavicIconsClass",         {bg = c.localtheme.accent[1], fg = c.localtheme.special.class[1]})
  vim.api.nvim_set_hl(0, "NavicIconsMethod",        {bg = c.localtheme.accent[1], fg = c.localtheme.brightteal[1]})
  vim.api.nvim_set_hl(0, "NavicIconsProperty",      {bg = c.localtheme.accent[1], fg = c.localtheme.orange[1]})
  vim.api.nvim_set_hl(0, "NavicIconsField",         {bg = c.localtheme.accent[1], fg = c.localtheme.orange[1]})
  vim.api.nvim_set_hl(0, "NavicIconsConstructor",   {bg = c.localtheme.accent[1], fg = c.localtheme.yellow[1]})
  vim.api.nvim_set_hl(0, "NavicIconsEnum",          {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsInterface",     {bg = c.localtheme.accent[1], fg = c.localtheme.purple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsFunction",      {bg = c.localtheme.accent[1], fg = c.localtheme.teal[1]})
  vim.api.nvim_set_hl(0, "NavicIconsVariable",      {bg = c.localtheme.accent[1], fg = c.localtheme.fg[1]})
  vim.api.nvim_set_hl(0, "NavicIconsConstant",      {bg = c.localtheme.accent[1], fg = c.localtheme.purple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsString",        {bg = c.localtheme.accent[1], fg = c.localtheme.string[1]})
  vim.api.nvim_set_hl(0, "NavicIconsNumber",        {bg = c.localtheme.accent[1], fg = c.localtheme.special.green[1]})
  vim.api.nvim_set_hl(0, "NavicIconsBoolean",       {bg = c.localtheme.accent[1], fg = c.localtheme.deepred[1]})
  vim.api.nvim_set_hl(0, "NavicIconsArray",         {bg = c.localtheme.accent[1], fg = c.localtheme.lpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsObject",        {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsKey",           {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsNull",          {bg = c.localtheme.accent[1], fg = c.localtheme.lpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsEnumMember",    {bg = c.localtheme.accent[1], fg = c.localtheme.orange[1]})
  vim.api.nvim_set_hl(0, "NavicIconsStruct",        {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsEvent",         {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicIconsOperator",      {bg = c.localtheme.accent[1], fg = c.localtheme.special[conf.special.operator][1]})
  vim.api.nvim_set_hl(0, "NavicIconsTypeParameter", {bg = c.localtheme.accent[1], fg = c.localtheme.darkpurple[1]})
  vim.api.nvim_set_hl(0, "NavicText",               {bg = c.localtheme.accent[1], fg = c.localtheme.fg_dim[1]})
  vim.api.nvim_set_hl(0, "NavicSeparator",          {bg = c.localtheme.accent[1], fg = c.localtheme.fg_dim[1]})
  c.link("NavbuddyNormalFloat", "NeoTreeNormalNC")
  c.link("NavbuddyFloatBorder", "TelescopeBorder")

  -- cokeline
  c.set_hl(0, "CokelineInactive", { bg = c.localtheme.statuslinebg[1], fg = c.cokeline_colors.fg })
  c.set_hl(0, "CokelineActive", { bg = c.theme.alt_accent_color, fg = c.theme.accent_fg })
  c.set_hl(0, "CokelineActiveModified", { bg = c.theme.alt_accent_color, fg = c.localtheme.special.red[1] })
  c.set_hl(0, "CokelineInactiveModified", { bg = c.localtheme.statuslinebg[1], fg = c.localtheme.special.red[1] })
end

local M = {}
function M.set()
  _set()
end

return M

