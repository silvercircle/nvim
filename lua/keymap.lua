...
--
-- <Home>: position cursor in the first col and refresh indent guides (possible bug)
...
kms('n', '<F2>', function()
  ...
  vim.api.nvim_input('<f16>')
  vim.schedule(schedule_mkview)
end, opts)
kms('i', '<F2>', function()
  local key = vim.api.nvim_replace_termcodes('<C-o>za', true, false, true)
  vim.api.nvim_feedkeys(key, 'i', false)
  vim.schedule(schedule_mkview)
end, opts)
kms('v', '<F2>', function()
  vim.api.nvim_feedkeys('zf', 'v', true)
  vim.api.nvim_input('<f16>')
  vim.schedule(schedule_mkview)
end, opts)
...
kms('i', '<f14>', function()
  local key = vim.api.nvim_replace_termcodes('<C-o>zc', true, false, true)
  vim.api.nvim_feedkeys(key, 'i', false)
  vim.schedule(schedule_mkview)
end, opts)
...
kms('i', '<f26>', function()
  local key = vim.api.nvim_replace_termcodes('<C-o>zo', true, false, true)
  vim.api.nvim_feedkeys(key, 'i', false)
  vim.schedule(schedule_mkview)
end, opts)
